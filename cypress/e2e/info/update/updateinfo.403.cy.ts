      ...
      (res) => {
        cy.request<CreateInfoRes>({
          ...
          cy.request({
            ...
            expect(resUpdateInfo.body.statusCode).to.eq(403);
            expect(resUpdateInfo.body.error).to.eq('Forbidden');
            expect(resUpdateInfo.body.message).to.eq('Forbidden resource');
          });
        });
      },
    );
